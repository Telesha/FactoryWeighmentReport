...
async function saveDispatchInvoice(values,dispatchList) {
  let saveModel = {
    ...
    dateOfDispatched: values.dateOfDispatched,
    sellingMarkID: parseInt(values.sellingMarkID),
    brokerID: parseInt(values.brokerID),
    dispatchNature: parseInt(values.natureOfDispatch),
...
async function updateDispatchInvoice(dispatchInvoiceModel) {
  dispatchInvoiceModel.createdBy = parseInt(tokenDecoder.getUserIDFromToken());
  dispatchInvoiceModel.createdDate = new Date().toISOString();
