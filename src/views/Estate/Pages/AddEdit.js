...
export default function EstateAddEdit() {
  ...
  const [title, setTitle] = useState("Location Registration");
  ...
  async function getFactoryDetails(factoryID) {
    ...
    if (response != null || response != undefined) {
      setGeneralDetailsArray({
        ...generalDetailsArray,
        operationEntityTypeID: response[0].operationEntityTypeID,
        ...
        labourOnBook: response[0].labourOnBook == null ? '' : response[0].labourOnBook,
        grantArea: response[0].grantArea == null ? '' : response[0].grantArea
      });
    ...
    if (oldData.length <= 0 || factoryAccountArray.length <= 0 || generalDetailsArray == null || (Object.keys(productDetailsArray.filter(x => x.isActive == true)).length <= 0)) {
      ...
      if (!isUpdate) {
        let saveModel = {
          ...
          labourOnBook: generalDetailsArray.labourOnBook,
          grantArea: parseFloat(generalDetailsArray.grantArea)
        }
      ...
      <Page className={classes.root} title="Factories Add Edit">
        <Container maxWidth={false}>
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                groupID: Yup.number().required('Legal Entity is required').positive('Select a Legal Entity'),
              })
            }
            onSubmit={saveUser}
            enableReinitialize
                      ...
                      <CardContent>
                        ...
                        <Grid container spacing={3}>
                          <Grid item md={4} xs={12}>
                            ...
                            >
                              <MenuItem value="0">--Business Division--</MenuItem>
                          ...
                          <Grid className={classes.root1} item xs={12}>
                            <AppBar position="static">
                              <Tabs value={value} onChange={handleTabChange} variant={'fullWidth'} classes={{ indicator: classes.indicator }}
                                ...
                                <Tab label="Garden Accounts" {...a11yProps(2)} style={{ color: "black" }} />
