...
export default function BuyerAddEdit(props) {
  ...
  async function getBuyerRegistrationDetails(buyerID) {
    ...
    let data = {
      ...
      joinedDate: response.joiningDate.split('T')[0],
  ...
  async function saveBuyerRegistration(values) {
    if (isUpdate == true) {
      let model = {
        ...
        registrationNumber: values.buyerReg,
        joiningDate: values.joinedDate,
          ...
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                ...
                buyerReg: Yup.string()
                  .max(10)
                  .required('Registration Number is required').matches(/^[0-9]{0,15}$/, 'Allow only numbers'),
                joinedDate: Yup.date()
                  .required('Joinng Date is required').typeError('Invalid date'),
                email: Yup.string()
