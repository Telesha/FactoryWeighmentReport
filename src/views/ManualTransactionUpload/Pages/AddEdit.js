...
export default function ManualTransactionUpload() {
    ...
    function checkIsMonthValid(result) {
        if (result.lastBalancePaymentSchedule !== null) {
            ...
            return (startOfMonth(Date.parse(year + '-' + (++month) + '-' + '01')));
        } else {
            return undefined
        }
    }
                    ...
                    <Formik
                        ...
                        validationSchema={
                            Yup.object().shape({
                                ...
                                transactionTypeID: Yup.string().test('amount', "Please select the transaction type", val => val > 0),
                                amount: Yup.string().matches(/^[0-9]+([.][0-9]{1,2})?$/, 'Please enter valid amount').test('amount', "Please provide valid amount", val => val > 0).required('Amount is required'),
                            })
                        }
                        enableReinitialize
