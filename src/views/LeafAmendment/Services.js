...
async function UpdateLeafAmendment(updateModel) {
  let updateModels = {
    updateNetWeightDailyCropBalance: updateModel.editModel,
    removeDailyCropBalance: updateModel.deleteModel,
    modifiedBy: tokenDecoder.getUserIDFromToken(),
  }
  const response = await CommonPost('/api/DailyCropAmending/UpdateLeafAmendment', null, updateModels);
  return response;
}
...
async function getLeafAmendmentDetails(groupID, factoryID, selectedDate, routeID, registrationNumber) {
  let model = { 
    groupID: parseInt(groupID),
    factoryID: parseInt(factoryID),
    selectedDate: selectedDate === undefined ? moment(new Date()).format() : moment(selectedDate).format(),
    routeID: parseInt(routeID) === 0 ? null : parseInt(routeID),
    registrationNumber: registrationNumber == "" ? null : registrationNumber
  }
  const response = await CommonPost('/api/DailyCropAmending/GetDailyCropBalanceDetails', null, model);
