...
export default function COPSectionCreation(data) {
  ...
  const [title, setTitle] = useState('COP Setup');
  ...
  async function SaveNewParentName() {
    ...
    if(data.data.groupID > 0 && data.data.factoryID > 0 ) {
      ...
      alert.error("Please select group and factory");
    }
  ...
  async function COPCreatedSectionDetails(parentsectionID) {
    ...
    if (respose.data !== null && respose.data.length > 0) {
      ...
      if (result[0].dataList[0].copChildSectionName == null) {

        alert.error('Please Configure Child Sections');
      }
    } else {
      alert.error('Please configure Child Section names');
