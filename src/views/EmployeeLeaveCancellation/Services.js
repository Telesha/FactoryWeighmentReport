...
async function getCostCenterDetailsByGardenID(estateID) {
...
async function GetApprovedLeaveRequestDetails(model) {
  const response = await CommonPost('/api/LeaveCancellation/GetApprovedLeaveRequestDetails', null, model);
  return response;
}

async function updateLeaveFormDetails(model) {
  const response = await CommonPost('/api/LeaveCancellation/UpdateLeaveFormDetails', null, model);
  return response;
}
