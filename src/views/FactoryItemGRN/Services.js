...
async function saveFactoryGrn(factoryGrn, grnList, dueDate, invoiceDate, receiveDate, factoryGRN) {
  let saveModel = {
    factoryItemGRNID: 0,
...
async function getPurchaseOrderListing(factoryID) {
  const response = await CommonGet('/api/PurchaseOrder/GetPurchaseOrderListing', 'factoryID=' + parseInt(factoryID))
