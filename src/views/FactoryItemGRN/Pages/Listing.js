...
export default function FactoryGRNListing() {
  ...
  const handleClick = () => {
    encrypted = btoa('0');
    navigate('/app/factoryGRN/addedit/' + encrypted);
  }

  const handleClickEdit = (factoryItemGRNID) => {
    encrypted = btoa(factoryItemGRNID.toString());
    navigate('/app/factoryGRN/addedit/' + encrypted);
  }
  ...
  async function getPermission() {
    ...
    var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWFACTORYITEMGRN');
            ...
            {({
              ...
              <form onSubmit={handleSubmit}>
                <Box mt={0}>
                  <Card>
                    <CardHeader
                      title={cardTitle("Goods Received Note")}
                    />
                    <PerfectScrollbar>
                      <Divider />
                      ...
                      <Box minWidth={1050}>
                        {factoryItemData.length > 0 ?
                          < MaterialTable
                            ...
                            columns={[
                              { title: 'Factory Item GRNID', field: 'factoryItemGRNID', hidden: true },
