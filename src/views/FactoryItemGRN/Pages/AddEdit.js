...
const screenCode = "FACTORYITEMGRN"

export default function FactoryGRNAddEdit(props) {
...
const [factoryGRN, setFactoryGRN] = useState({
  ...
  const handleClick = () => {
    ...
    navigate('/app/factoryGRN/listing');

  }
  const alert = useAlert();
  const { factoryItemGRNID } = useParams();
  let decrypted = 0;

  useEffect(() => {
    decrypted = atob(factoryItemGRNID.toString());
    if (decrypted != 0) {
      trackPromise(
...
async function getGroupsForDropdown() {
  ...
  async function getPermission() {
    ...
    var isAuthorized = permissions.find(p => p.permissionCode == 'ADDEDITFACTORYITEMGRN');
  ...
  async function getItemDetails(factoryItemGRNID) {

    let response = await services.getGRNDetailsByID(factoryItemGRNID);
    let data = response;
  ...
  async function saveFactoryGrn(values) {
    if (factoryGRN.grnType == 1 && factoryGRN.supplierID == 0) {
      alert.error("please select the supplier");
    }
    ...
    if (isUpdate == true) {
      ...
      let response = await services.updateFactoryItem(atob(factoryItemGRNID.toString()), factoryGRN, grnData
      ...
      if (factoryGRN.grnType == 2) {
        ...
        if (count > 0) {
          ...
          if (tableItems.length >= 2) {
            alert.error("Only select a one PO Number");
          }
          else {
            if (factoryGRN.poNumber != -1) {
              let response = await services.saveFactoryGrn(values, tableItems, dueDate, invoiceDate, receiveDate, factoryGRN);
              if (response.statusCode == "Success") {
                alert.success(response.message);
                setIsDisableButton(true);
                navigate('/app/factoryGRN/listing');
          ...
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                ...
                invoiceNumber: Yup.string().required('Invoice number required').matches(/^[0-9a-zA-Z_@./#&+-]+$/, 'Only allow alphanumeric'),
                //supplierID: Yup.number().required('Supplier name required').min("1", 'Supplier name required'),
                // paymentTypeID: Yup.number().required('Payment type required').min("1", 'Payment type required')                
              })
            }
            onSubmit={saveFactoryGrn}
            enableReinitialize
          >
            {({
              errors,
                      ...
                      <CardContent>
                        ...
                        {factoryGRN.grnType == 1 || (factoryGRN.grnType == 2 && isUpdate == true) ?
                          <Formik
                            ...
                            validationSchema={
                              Yup.object().shape({
                                itemCategoryID: Yup.number().required('Item category required').min("1", 'Item category required'),
                                factoryItemSupplierID: Yup.number().required('Item required').min("1", 'Item required'),
