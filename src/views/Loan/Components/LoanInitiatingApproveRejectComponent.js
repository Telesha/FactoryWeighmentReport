...
export const LoanInitiating = ({
    ...
    async function ApproveLoanRequest() {
        ...
        let approveObject = {
            customerLoanID: FullLoanDetails.customerLoanID,
            modifiedBy: tokenDecoder.getUserIDFromToken(),
            remarks: Remarks === "" ? null : ("Approve :- " + Remarks),
            result: -1
        }

        const resposnse = await Services.ApproveLoanRequest(approveObject);
        if (resposnse.statusCode == "Success") {
            alert.success(resposnse.message);
            setTimeout(() => { navigate('/app/loan/listing'); }, 1000)
        }
        else {
    ...
    async function RejectLoanRequest() {
        ...
        const resposnse = await Services.RejectLoanRequest(rejectObject);
        if (resposnse.statusCode == "Success") {
            alert.success(resposnse.message);
            setTimeout(() => { navigate('/app/loan/listing'); }, 1000)
        }
    ...
    async function IssueLoan() {
        ...
        const resposnse = await Services.IssueLoanRequest(issueObject);
        if (resposnse.statusCode == "Success") {
            alert.success(resposnse.message);
            setTimeout(() => { navigate('/app/loan/listing'); }, 1000)
        }
        ...
        <Formik
            ...
            validationSchema={
                Yup.object().shape({
                    ...
                    customerRegistrationNumber: Yup.string().matches(/^[0-9]+$/, 'Entered Registration number not valid').required("Registration number is required"),
