...
export default function CropBulkUploadAddEdit(props) {
  ...
  function handleClickEdit(id) {
    var filterdData = cropData.filter(x => x.id !== id);
    setCropData(filterdData);
  }
                              ...
                              onRowUpdate: (newData, oldData) =>
                                new Promise((resolve, reject) => {
                                  setTimeout(() => {
                                    ...
                                    dataUpdate[index] = {
                                      weightReadTime: newData.weightReadTime == oldData.weightReadTime ? newData.weightReadTime : moment(newData.weightReadTime.toISOString().split("T")[0]).format('M/D/YYYY'),
