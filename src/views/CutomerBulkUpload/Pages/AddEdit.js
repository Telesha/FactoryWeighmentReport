...
export default function CutomerBulkUpload(props) {
  ...
  async function saveCustomer() {
    let datarest = await Promise.all(
      customerData.map(async data => {
        var result = customerData.filter(
          a => a.registrationNumber === data.registrationNumber
        );
        var format = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]+/;
        if (data.paymentTypeID > 3 || data.paymentTypeID == 0) {
          return data;
        } else if (data.customerCode.length != data.customerCode.length) {
          return data;
        } else if (
          data.nic.length < 10 ||
          data.nic.length == 11 ||
    ...
    datarest.map(itemx => {
      if (itemx != undefined) {
        erroredList.push(itemx);
        errorCount++;
      }
    });
    setcustomerData(erroredList);
    if (errorCount === 0) {
      alert.success('Customer details uploaded successfully');
    } else {
      if (datarest.length - errorCount == 0) {
        alert.error('CSV details not uploaded.');
      } else {
        alert.success(
          datarest.length -
            errorCount +
        ...
        alert.error('Error occured in ' + errorCount + ' record(s) uploading.');
      }
    }
  }
