...
const screenCode = 'BUYERWISESALESREPORT';

export default function BuyerwiseSalesReport(props) {
    const [title, setTitle] = useState('Buyer Wise Grade Sales Report');
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(
            p => p.permissionCode == 'VIEWBUYERWISESALESREPORT'
        );
    ...
    async function getBuyerWiseSalesReportData() {
        ...
        const buyerSalesData = await services.getBuyerwiseGradeSalesReport(model);
        getSelectedDropdownValuesForReport(model);

        if (buyerSalesData.statusCode == 'Success' && buyerSalesData.data != null) {

            setReportData(buyerSalesData.data);

            const dataList = buyerSalesData.data;
            dataList.forEach(x => {
                let salesList = x.buyerWiseGradeSalesList;
                salesList.forEach(y => {

                    finalTotalQty = y.netWeight + finalTotalQty;
                    finalTotalAmount = y.value + finalTotalAmount;
            ...
            dataList.forEach(x => {
                ...
                salesList.forEach(y => {
                    individualQty = y.netWeight + individualQty;
                    individualTotalAmount = y.value + individualTotalAmount;

                });

                salesList.netWeight = individualQty;
                salesList.value = individualTotalAmount;

                setIndividualTotal({
                    ...individualTotal,
                    qtyTotal: individualQty,
                    valueTotal: individualTotalAmount
                })
