...
async function GetDetails(model) {
  let response = await CommonPost('/api/LabourChecklistReport/GetDailyWeighmentRegisteryDetails', null, model);
...
async function GetFactoryJobs(date) {
  var response = await CommonGet('/api/Task/GetHarvestingJobsByDate', 'date=' + new Date(date).toISOString());
  return response.data;
}
