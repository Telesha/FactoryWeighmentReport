...
export default {
  getAllGroups,
  GetWorkerAttendanceNonPlukingReportDetails,
...
async function GetWorkerAttendanceNonPlukingReportDetails(model) {
  let response = await CommonPost('/api/ReconcilationReport/GetDetailsRecinsiliationReport', null, model);
  return response;
}
...
async function getLabourTask(operationEntityID, divisionID, date) {
  const response = await CommonGet('/api/Task/GetPluckingTaskNamesByOperationID', 'gardenID=' + parseInt(operationEntityID))
  return response.data;
}
...
async function GetSirdersByDivisionID(divisionID) {
  var response = await CommonGet('/api/Employee/GetSirdersByDivisionID', "divisionID=" + parseInt(divisionID));
  return response.data;
}

async function GetOperatorListByDateAndGardenIDForLabourChecklistReport(factoryID, date) {
  let response = await CommonGet('/api/User/GetOperatorListByDateAndGardenIDForReconcilationReport', "factoryID=" + parseInt(factoryID) + "&date=" + date);
