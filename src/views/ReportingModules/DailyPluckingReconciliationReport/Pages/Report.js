...
const screenCode = 'DAILYPLUCKINGRECONCILIATIONREPORT';

export default function DailyPluckingReconciliationReport(props) {
    const [title, setTitle] = useState("Daily Plucking Reconciliation Report")
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWPLUCKINGRECONCILIATIONREPORT');
    ...
    async function createDataForExcel(array) {
        ...
        if (array != null) {
            ...
            var vr = {
                ...
                'Emp.Name': selectedSearchValues.empTypeID == "" ? 'EmployeeSubCategory - All Employee Sub Categories' : 'Employee Sub Category - ' + selectedSearchValues.empTypeID,
            ...
            res.push(vr);
            var vr1 = {
                'Kamjari': 'Date - ' + moment(selectedSearchValues.date.toString()).format().split('T')[0]
            }
            res.push(vr1);
        }
        return res;
    ...
    async function createFile() {
        ...
        var settings = {
            ...
            fileName:
                'Daily Reconciliation Report ' + ' - ' + selectedSearchValues.gardenName + ' - ' + selectedSearchValues.costCenterName + ' - ' + moment(selectedSearchValues.date.toString()).format().split('T')[0]
        };
                                    ...
                                    <PerfectScrollbar>
                                        ...
                                        {attendanceData.length > 0 ?
                                            <Box display="flex" justifyContent="flex-end" p={2}>
                                                ...
                                                <ReactToPrint
                                                    documentTitle={"Daily Plucking Reconciliation Report" + ' - ' + selectedSearchValues.gardenName + ' - ' + selectedSearchValues.costCenterName + ' - ' + moment(selectedSearchValues.date.toString()).format().split('T')[0]}
                                                    trigger={() => <Button
                                                        color="primary"
                                                        id="btnCancel"
                                                        variant="contained"
