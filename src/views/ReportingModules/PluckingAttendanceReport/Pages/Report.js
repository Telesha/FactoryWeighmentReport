...
const screenCode = 'PLUCKINGATTENDANCEREPORT';

export default function PluckingAttendanceReport(props) {
    const [title, setTitle] = useState("Plucking Attendance Record Export")
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWPLUCKINGATTENDANCEREPORT');
    ...
    async function GetDetails() {
        ...
        const response = await services.GetPluckingAttendanceDetails(model);
        ...
        if (array != null) {
            array.map(x => {
                var vr = {
                    ...
                    'FullorHalf': null,
                    'GLorLatex': (x.quntity != null && x.quntity !== '') ? x.quntity : "NULL",
                                                        ...
                                                        <Table className={classes.table} aria-label="simple table" size='small'>
                                                            <TableHead>
                                                                <TableRow>
                                                                    ...
                                                                    <TableCell align="right" style={{ fontWeight: "bold", border: "1px solid black" }}>FullorHalf</TableCell>
                                                                    <TableCell align="right" style={{ fontWeight: "bold", border: "1px solid black" }}>GLorLatex</TableCell>
                                                            ...
                                                            <TableBody>
                                                                {attendanceData.slice(page * limit, page * limit + limit).map((row, i) => (
                                                                    <TableRow key={i}>
                                                                        ...
                                                                        <TableCell component="th" scope="row" align="right" style={{ border: "1px solid black" }}> </TableCell>
                                                                        <TableCell component="th" scope="row" align="right" style={{ border: "1px solid black" }}>{row.quntity || 'NULL'} </TableCell>
