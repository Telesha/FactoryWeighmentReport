...
const screenCode = 'WEEKLYOUTSIDERPAYMENTREPORT';

export default function WeeklyOutsiderPaymentReport(props) {
    const [title, setTitle] = useState("Weekly Outsider Payment Report")
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWWEEKLYOUTSIDERPAYMENTREPORT');
    ...
    async function GetDetails() {
        ...
        if (response.statusCode == "Success" && response.data != null) {
            ...
            groupedData.forEach(x => {
                ...
                x.totalNet = (x.totalGross + x.cfAmount) - x.ration
    ...
    async function createFile() {
        ...
        var settings = {
            sheetName: 'Weekly Outsider Payment Report',
            fileName:
                'Weekly Outsider Payment Report - ' +
        ...
        let dataA = [
            {
                sheet: 'Weekly Outsider Payment Report',
                                    ...
                                    <PerfectScrollbar>
                                        ...
                                        {dailylabourData.length > 0 ?
                                            <Box display="flex" justifyContent="flex-end" p={2}>
                                                ...
                                                <div>&nbsp;</div>
                                                <ReactToPrint
                                                    documentTitle={"Weekly Outsider Payment Report"}
                                                    trigger={() => <Button
                                                        color="primary"
                                                        id="btnCancel"
                                                        variant="contained"
