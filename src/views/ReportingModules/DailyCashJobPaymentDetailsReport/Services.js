...
async function GetOffDayCashPaymentDetails(model) {
  ...
  let response = await CommonPost('/api/DailyCashJobPaymentDetailsReport/GetDailyCashJobPaymentDetailsReportData', null, newmodel);
  return response;
}
...
async function GetFactoryJobs(factoryId, date) {
  var response = await CommonGet('/api/Task/GetFactoryJobsByDateCashJob', 'factoryId=' + parseInt(factoryId) + '&date=' + date);
  let array = []
  for (let item of Object.entries(response.data)) {
...
async function getFieldDetailsByDivisionID(divisionID, date) {
  let response = await CommonGet('/api/Field/getFieldDetailsByDivisionIDDateCashJob', "divisionID=" + parseInt(divisionID) + '&date=' + date);
  let fieldArray = [];
  for (let item of Object.entries(response.data)) {
...
async function getEmployeeTypesWithoutOutsiderForDropdown() {
  var response = await CommonGet('/api/DailyCashJobPaymentDetailsReport/GetEmployeeTypeWithoutOutsider', null);
  let array = []
  for (let item of Object.entries(response.data)) {
    array[item[1]["employeeTypeID"]] = { name: item[1]["employeeTypeName"], code: item[1]["employeeTypeCode"] }
  }
...
async function getOperatorsForDropdown(date) {
  var response = await CommonGet('/api/DailyCashJobPaymentDetailsReport/GetOperatorsForDropdown', "date=" + date);
  return response.data;
}
