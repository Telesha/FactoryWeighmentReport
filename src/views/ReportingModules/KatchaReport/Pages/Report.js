...
const screenCode = 'KATCHAREPORT';

export default function KatchaReport(props) {
    const [title, setTitle] = useState("Weekly Katcha Report")
    ...
    const [enabledDay, setEnabledDay] = useState(1);//Sunday is 0, Monday is 1, and so on up to Saturday, which is 6.
    const [startDay, setStartDay] = useState('Mon');
    const [holiday, setHoliday] = useState('Sun');

    const isDayDisabledM = (date) => {
        if (enabledDay === null) {
    ...
    if (prescription.prescriptionDate instanceof Date) {
        date = new Date(new Date(prescription.prescriptionDate).setDate(prescription.prescriptionDate.getDate() + prescription.prescriptionExpirationDate)).toISOString();
    }
    ...
    useEffect(() => {
        trackPromise(
            GetWeeklyPaymentMasterByEstateID()
        )
    }, [katchaDataList.gardenID], [katchaDataList.groupID]);
    ...
    useEffect(() => {
        if (katchaDataList.costCenterID != "0") {
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWKATCHAREPORT');
    ...
    async function GetWeeklyPaymentMasterByEstateID() {
        var response = await services.GetWeeklyPaymentMasterByEstateID(katchaDataList.gardenID);
    ...
    async function getGangDetailsByDivisionID() {
        var response = await services.getGangDetailsByDivisionID(katchaDataList.costCenterID);
        var newOptionArray = [];
    ...
    async function GetDetails() {
        ...
        const response = await services.GetKatchaReportDetails(model);
    ...
    function createReportTableData(data) {
        ...
        data.forEach((x, i) => {
            ...
            x.employeeTypeList.forEach(y => {
                ...
                totalAddition = totalAddition + y.addition;
                totalDeduction = totalDeduction + y.deduction;
                grndTotal = grndTotal + (y.allowance + y.basicAmount + y.gardenAllowance + y.otAmount + y.addition - y.deduction);
                ...
                x.totalAddition = totalAddition
                x.totalDeduction = totalDeduction
                ...
                y.duffaTypeList.forEach(z => {
                    ...
                    var colname5 = y.empTypeID + ' addition';
                    var colname6 = y.empTypeID + ' deduction';
                    ...
                    if (duplicateDate) {
                        ...
                        duplicateDate[colname5] = duplicateDate[colname5] == undefined ? 0 + y.addition : duplicateDate[colname5];
                        duplicateDate[colname6] = duplicateDate[colname6] == undefined ? 0 + y.deduction : duplicateDate[colname6];
                ...
                result[i].totalAddition = x.totalAddition
                result[i].totalDeduction = x.totalDeduction
    ...
    function createReportTableColumnNames(data) {
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            resultTwo.push({ empTypeID: result[i].empTypeID, empTypeName: result[i].empTypeName, colID: result[i].empTypeID + ' deduction', colName: 'Deduction' })
            resultTwo.push({ empTypeID: result[i].empTypeID, empTypeName: result[i].empTypeName, colID: result[i].empTypeID + ' addition', colName: 'Addition' })
        ...
        resultThree.push({ colID: 'totalAddition' })
        resultThree.push({ colID: 'totalDeduction' })
    ...
    async function createDataForExcel(data) {
        ...
        if (array != null) {
            ...
            var vr2 = {
                'kamjariName': 'Cost Center - ' + selectedSearchValues.costCenterName,
            };
            var vr3 = {
    ...
    async function createFile() {
        ...
        var settings = {
            sheetName: 'Weekly Katcha Report',
            fileName: 'Weekly Katcha Report - ' + fromDate.toISOString().substr(0, 10) + ' - ' + date.substr(0, 10)
        };
        ...
        res.push({ label: 'Total Addition', value: 'totalAddition' });
        res.push({ label: 'Total Deduction', value: 'totalDeduction' });
        res.push({ label: 'Grand Total', value: 'grndTotal' });

        let dataA = [
            {
                sheet: 'Weekly Katcha Report',
                columns: res,
                content: file
            }
        ];
                                    ...
                                    <PerfectScrollbar>
                                                ...
                                                <Grid item md={3} xs={12}>
                                                    ...
                                                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                                        <KeyboardDatePicker
                                                            ...
                                                            id="fromDate"
                                                            value={fromDate}
                                                            shouldDisableDate={isDayDisabledM}
                                                            onChange={(e) => {
                                                                setFromDate(e);
                                        ...
                                        {katchaData.length > 0 ?
                                            <Box display="flex" justifyContent="flex-end" p={2}>
                                                ...
                                                <ReactToPrint
                                                    documentTitle={"Weekly Katcha Report - " + fromDate.toISOString().substr(0, 10) + ' - ' + date.substr(0, 10)}
                                                    trigger={() => <Button
                                                        color="primary"
                                                        id="btnCancel"
                                                        variant="contained"
