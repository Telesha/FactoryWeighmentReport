...
async function GetDailyLabourDetails(model) {
  let response = await CommonPost('/api/WeeklyOutsiderNonPluckingPaymentReport/GetWeeklyOutsiderNonPluckingPaymentReportData', null, model);
  return response;
}
...
async function getBookDetailsForDropDown() {
  let response = await CommonGet('/api/Book/GetNonPluckingBookDetailsByEmployeeTypeForDropDown', null);
  return response.data;
};
