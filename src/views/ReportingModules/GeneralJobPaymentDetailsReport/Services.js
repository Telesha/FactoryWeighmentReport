...
async function GetOffDayCashPaymentDetails(model) {
  let newmodel = {
    ...
    HarvestJobTypeID: (model.harvestJobTypeID),
    EmployeeTypeID: (model.employeeTypeID),
    Date: new Date(model.applicableDate),
  }
  let response = await CommonPost('/api/DailyGeneralJobDetail/GetGeneralJobPaymentDetailsReport', null, newmodel);
  return response;
}
...
async function GetFactoryJobs(factoryId, date) {
  var response = await CommonGet('/api/Task/GetFactoryJobsGenaralJob', 'factoryId=' + parseInt(factoryId) + '&date=' + date);
  let array = []
  for (let item of Object.entries(response.data)) {
...
async function GetAllEmployeeSubCategoryMapping() {
  var response = await CommonGet('/api/Employee/GetAllEmployeeSubCategoryMapping', null);
