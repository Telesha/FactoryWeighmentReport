...
const screenCode = 'DAILYNONPLUCKINGJOBPAYMENTDETAILSREPORT';

export default function DailyNonPluckingJobPaymentDetailsReport(props) {
    const [title, setTitle] = useState("Daily Non Plucking Job Payment Details Report")
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWDAILYNONPLUCKINGJOBPAYMENTSDETAILSREPORT');
    ...
    async function getFieldDetailsByDivisionID() {
        var response = await services.getFieldDetailsByDivisionID(dailyNonPluckingPaymentList.costCenterID);
        setFields(response);
    };
    ...
    async function GetDetails() {
        ...
        let model = {
            ...
            applicableDate: new Date(dailyNonPluckingPaymentList.date),
            harvestJobTypeID: parseInt(dailyNonPluckingPaymentList.factoryJobID),
            fieldID: parseInt(dailyNonPluckingPaymentList.fieldID),
            employeeTypeID: parseInt(dailyNonPluckingPaymentList.empTypeID)
        }
    ...
    function calculateTotalQty() {
        const totalAmount = dailyNonPluckingPaymentData.reduce((accumulator, current) => accumulator + current.totalAmount, 0);
        ...
        const totalOTAmount = dailyNonPluckingPaymentData.reduce((accumulator, current) => accumulator + current.otAmount, 0);
        const totalDeductionAmount = dailyNonPluckingPaymentData.reduce((accumulator, current) => accumulator + current.deductionAmount, 0);
        setTotalValues({
            ...totalValues,
        ...
        if (array != null) {
            array.map(x => {
                var vr = {
                    ...
                    'Gross(BDT)': x.grossAmount.toFixed(2),
                    'Allowance(BDT)': x.allowance.toFixed(2),
                    'PF Deduction(BDT)': x.pfDeductionAmount.toFixed(2),
                    'Total(BDT)': x.totalAmount.toFixed(2)
                };
                res.push(vr);
            });
            var vr = {
                'Reg.No': 'Total',
                'Gross(BDT)': totalValues.totalgrossAmount.toFixed(2),
                'Allowance(BDT)': totalValues.totalallowance.toFixed(2),
                'PF Deduction(BDT)': totalValues.totalpfDeductionAmount.toFixed(2),
                'Total(BDT)': totalValues.totalAmount.toFixed(2),
            ...
            var vr = {
                ...
                'Gross(BDT)': 'Date: ' + selectedSearchValues.date,
                'Allowance(BDT)': selectedSearchValues.empTypeName === undefined ? 'Employee Type: All Employee Types' : 'Employee Type: ' + selectedSearchValues.empTypeName
            };
            res.push(vr);
        }
    ...
    async function createFile() {
        ...
        var settings = {
            sheetName: 'Non Job Payment',
            fileName:
                'Daily Non Plucking Job Payment Details Report - ' +
                                    ...
                                    <PerfectScrollbar>
                                            ...
                                            <Grid container spacing={3}>
                                                ...
                                                <Grid item md={4} xs={8}>
                                                    ...
                                                    <TextField select
                                                        ...
                                                        onBlur={handleBlur}
                                                        onChange={(e) => handleChange(e)}
                                                        value={dailyNonPluckingPaymentList.empTypeID}
                                                        variant="outlined"
                                                        id="empTypeID"
                                                        size='small'
                                                ...
                                                <Grid item md={4} xs={8}>
                                                    ...
                                                    <TextField
                                                        ...
                                                        type="date"
                                                        onChange={(e) => handleChange(e)}
                                                        value={dailyNonPluckingPaymentList.date}
                                                        variant="outlined"
                                                        id="date"
                                                    />
                                                </Grid>
                                        ...
                                        {dailyNonPluckingPaymentData.length > 0 ?
                                            <Box display="flex" justifyContent="flex-end" p={2}>
                                                ...
                                                <div>&nbsp;</div>
                                                <ReactToPrint
                                                    documentTitle={"Daily Non Plucking Job Payment Details Report"}
                                                    trigger={() => <Button
                                                        color="primary"
                                                        id="btnCancel"
                                                        variant="contained"
