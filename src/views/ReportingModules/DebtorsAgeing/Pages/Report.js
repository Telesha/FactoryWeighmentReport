...
const screenCode = 'DEBTORSAGEING';

export default function DebtorsAgeing(props) {
  ...
  const [title, setTitle] = useState("Debtors Ageing Report");
  ...
  async function getPermission() {
    ...
    var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWBEBTORSAGEING');
    ...
    var isBalanceSheetConfigurationEnabled = permissions.find(p => p.permissionCode == 'ENABLEDEBTORSAGEINGCONFIGURATION');
  ...
  async function GetDebtorsAgeingDetails() {

    if (DebtorsAgeingDetails.groupID === "0" || DebtorsAgeingDetails.factoryID === "0") {
      return;
    }

    const balanceSheetCongiDetails = await GetDebtorsAgeingSetupConfigurationDetails();
    ...
    const result = await services.GetDebtorsAgeingDetails(model);
      ...
      x.complexObj.forEach(y => {
        if (y.debit > 0) {
          var detailModel = {
            ...
            total1: y.coloumCount == 1 ? y.debit : null,
            total2: y.coloumCount == 2 ? y.debit : null,
            total3: y.coloumCount == 3 ? y.debit : null,
            total4: y.coloumCount == 4 ? y.debit : null
          };
          alltotal = alltotal + y.debit;
          if (y.coloumCount == 1) {
            total1 = total1 + y.debit;
          }
          else if (y.coloumCount == 2) {
            total2 = total2 + y.debit;
          }
          else if (y.coloumCount == 3) {
            total3 = total3 + y.debit;
          }
          else if (y.coloumCount == 4) {
            total4 = total4 + y.debit;
          }
          returnDataArray.push(detailModel);
        }
      })
  ...
  async function GetDebtorsAgeingSetupConfigurationDetails() {
    const result = await services.GetDebtorsAgeingSectionSetupConfigurationDetails(parseInt(DebtorsAgeingDetails.groupID.toString()), parseInt(DebtorsAgeingDetails.factoryID.toString()));
    return result;
  }

  function NavigateToConfigurationPage() {
    navigate('/app/DebtorsAgeing/DebtorsAgeingSetupConfiguration/' + btoa(DebtorsAgeingDetails.groupID) + "/" + btoa(DebtorsAgeingDetails.factoryID));
  }
            ...
            <>
              {
                permissionList.isBalanceSheetConfigurationEnabled === true ?
                  <Grid item md={1} xs={12}>
                    <Tooltip title="Debtors Ageing Configurations">
                      <IconButton
  ...
  async function createFile() {
    ...
    var settings = {
      sheetName: 'Debtors Ageing Report',
      fileName: 'Debtors Ageing Report',
      writeOptions: {}
    }
                      ...
                      <CardContent>
                        ...
                        <Box display="flex" flexDirection="row-reverse" p={2} >
                          <Button
                            ...
                            type="submit"
                            variant="contained"
                            onClick={() => trackPromise(GetDebtorsAgeingDetails())}
                            size='small'
                          >
                            Search
                          </Button>
                        ...
                        {reportData.length > 0 ?
                          <Box display="flex" justifyContent="flex-end" p={2}>
                            ...
                            <ReactToPrint
                              documentTitle={"Debtors Ageing Report"}
                              trigger={() => <Button
                                color="primary"
                                id="btnRecord"
                                type="button"
