...
const screenCode = 'DAILYOUTSIDERWAGESREPORT';

export default function DailyOutsiderWagesReport(props) {
    const [title, setTitle] = useState("Daily Outsider Wages Report")
    ...
    useEffect(() => {
        trackPromise(getGroupsForDropdown(), getPermission(), getJob());
    }, []);
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWDAILYOUTSIDERWAGESREPORT');
    ...
    function calculateTotalQty() {
        ...
        const totalAmount = offDayCashPaymentData.reduce((accumulator, current) => accumulator + current.amount + current.allowance + current.extraPayment, 0);
        setTotalValues({
            ...totalValues,
        ...
        if (array != null) {
            array.map(x => {
                var vr = {
                    ...
                    'Allowances(BSC)': x.allowance.toFixed(2),
                    'Extra Payment': x.extraPayment.toFixed(2),
                    'Total Amount': (x.amount + x.allowance + x.extraPayment).toFixed(2)
                };
                res.push(vr);
            });
            var vr = {
                ...
                'Allowances(BSC)': totalValues.totalallowance.toFixed(2),
            ...
            var vr = {
                ...
                'Emp.Type': selectedSearchValues.statusID === "" ? 'Job Type : All' : 'Job Type :' + selectedSearchValues.statusID,
                'Job Type': 'Date : ' + selectedSearchValues.date
            };
            res.push(vr);
        }
    ...
    async function createFile() {
        ...
        var settings = {
            sheetName: 'Daily Outsider Wages Details',
            fileName:
                'Daily Outsider Wages Details Report - ' +
        ...
        let dataA = [
            {
                sheet: 'Daily Outsider Wages Details',
