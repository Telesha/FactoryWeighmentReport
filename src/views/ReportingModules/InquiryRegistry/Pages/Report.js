...
import {
  ...
  MenuItem, InputLabel, Chip, Typography, responsiveFontSizes
...
export default function InquiryRegistry(props) {
    ...
    if (array != null) {
      array.map(x => {
        var vr = {
          ...
          Debit: x.debit.toLocaleString(),
          Credit: x.credit.toLocaleString(),
        }
        res.push(vr);
      });
    }
  ...
  function viewLedgerTransactions(referenceNumber) {
    let encrypted = btoa(referenceNumber.toString());
    navigate('/app/generalJournal/view/' + encrypted);
  }
                        ...
                        {transactionDetails.length > 0 ?
                          <MaterialTable
                            ...
                            columns={[
                              ...
                              { title: 'Debit(Rs.)', field: 'debit', render: (rowData) => rowData.debit.toLocaleString() },
                              { title: 'Credit(Rs.)', field: 'credit', render: (rowData) => rowData.credit.toLocaleString() },
                            ]}
