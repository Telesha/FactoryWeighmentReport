...
async function GetDetails(model) {
  let response = await CommonPost('/api/EmployeeAvailed/GetEmployeeDetailsForLeaveRequest', null, model);
...
async function GetAllLeaveTypes() {
  var response = await CommonGet('/api/EmployeeAvailed/GetAllLeaveTypes', null);
  let array = []
  for (let item of Object.entries(response.data)) {
    array[item[1]["leaveTypeConfigurationID"]] = item[1]["shortForm"]
  }
  return array;
}