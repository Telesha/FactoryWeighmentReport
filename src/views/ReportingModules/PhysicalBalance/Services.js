...
async function UpdateOverAdvanceRequest(data) {
  ...
  let approveModel = {
    physicalbalanceID: parseInt(data.physicalbalanceID),
...
async function getApprovedDetailsByID(physicalbalanceID) {
  const response = await CommonGet('/api/AdvancePaymentApproval/GetAdvancePaymentApprovedDetailsByID', "physicalbalanceID=" + parseInt(physicalbalanceID));
...
async function getAdvancePaymentRequestDetails(physicalbalanceID) {
  const response = await CommonGet('/api/AdvancePayment/GetAdvancePaymentRequestDetails', "physicalbalanceID=" + parseInt(physicalbalanceID));
  return response.data;
}

async function updateAdvancePayment(Apayment) {
  let updateModel = {
    physicalbalanceID: parseInt(Apayment.physicalbalanceID),
