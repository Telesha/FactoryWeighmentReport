        ...
        <Formik
          ...
          validationSchema={
            Yup.object().shape({
              ...
              address1: Yup.string().max(255).matches(/^[a-zA-Z0-9\d\s\,\.\/]+$/, 'Special characters are not allowed').required('Address 1 required').max('20', 'Allow only 20 digits'),
              address2: Yup.string().max(255).matches(/^[a-zA-Z0-9\d\s\,\.\/]+$/, 'Special characters are not allowed').required('Address 2 required').max('20', 'Allow only 20 digits'),
              address3: Yup.string().max(255).matches(/^[a-zA-Z0-9\d\s\,\.\/]+$/, 'Special characters are not allowed').nullable().max('20', 'Allow only 20 digits'),
              zipCode: Yup.string().max(5, 'Only allow 5 digits').matches(/^[0-9\b]+$/, 'Only allow numbers').min('5', 'Only allow 5 digits').nullable(),
              nicBRNumber: Yup.string().required('NIC/BR Number Required').matches(/^[A-Z a-z 0-9_-]*$/, 'Special characters are not allowed').max('12', 'Maximum 12 characters allowed'),
              officePhoneNumber: Yup.string().matches(/^((\\+[1-9]{1,4}[ \\-]*)|(\\([0-9]{2,3}\\)[ \\-]*)|([0-9]{2,4})[ \\-]*)*?[0-9]{3,4}?[ \\-]*[0-9]{3,4}?$/, 'Please enter valid phone number').
                min(10, "Enter number is too short").max(10, "Entered number is too long").required('Office phone number required'),
              ...
              supplierName: Yup.string().required('Supplier Name required').matches(/^[a-zA-Z0-9_ ]*$/, 'Special characters are not allowed').max('50', 'Allow only 50 digits'),
            })
          }
                    ...
                    <CardContent>
                      <Grid container spacing={3}>
                        ...
                        <Grid item md={4} xs={12}>
                          ...
                          <Switch
                            checked={values.isCreditSupplier}
