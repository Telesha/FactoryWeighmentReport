  ...
  async function confirmData() {
    ...
    if (paymentArray[tableIndex].factoryItemSupplierPaymentID != undefined || paymentArray[tableIndex].factoryItemSupplierPaymentID != null) {
      ...
      if (res == 1) {
        ...
        alert.error('Error occured in item removing');
        setDialogbox(false);
      }
    }
    else {
            ...
            validationSchema={
              Yup.object().shape({
                ...
                accountNumber: Yup.string().when("branchID",
                  {
                    is: val => val > 0,
                    then: Yup.string().required('Please enter account number').max(20, "Maximum allow 20 digits").matches(/^[0-9\b]+$/, 'Only allow numbers'),
                  }),
                accountHolderName: Yup.string().when("branchID",
                  {
                    is: val => val > 0,
                    then: Yup.string().required('Please enter account holder name').max(20, "Maximum allow 20 digits"),
                  }),
              })
            }
