      ...
      <Page className={classes.root} title="General Details Add Edit">
        <Container maxWidth={false}>
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                factoryCode: Yup.string().max(255).required('Operation Entity code is required').matches(/^[0-9\b]+$/, 'Operation Entity code must be a positive number')
                  .min(2, 'Operation Entity code must be at least 2 characters').max(2, 'Operation Entity Code must be at most 2 characters'),
                factoryName: Yup.string().max(255).required('Operation Entity name is required').max(50, 'Allow only 50 digits').matches(/^(?!\s)/, 'Only alphabets and special characters are allowed'),
                managerName: Yup.string().max(255).nullable().matches(/^(?!\s)/, 'Only alphabets and special characters are allowed').max(50, 'Allow only 50 digits'),
                ...
                brNumber: Yup.string().max(255).nullable().matches(/^(?!\s)/, 'Only alphabets and special characters are allowed'),
