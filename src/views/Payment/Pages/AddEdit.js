...
export default function PaymentAddEdit(props) {
  ...
  async function getPermissions() {
    ...
    var isApproveButtonEnabled = permissions.find(
      p => p.permissionCode == 'PAYMENTAPPROVEBUTTONENABLED'
    );
  ...
  function concatenate(voucherType, transactionMode) {
    ...
    const transactionCode = transactionModes.find(
      x => x.transactionModeID === transactionMode
    );
    const concatCode = transactionCode.transactionModeCode.concat(
  ...
  function onKeyDown(e, id, data) {
    if (e.key === 'Tab') {
      if (data.credit != 0 || data.debit != 0) {
        if (
          journalData[journalData.length - 1].rowID.toString() === id.toString()
        ) {
                  ...
                  <Card>
                    ...
                    <PerfectScrollbar>
                                  ...
                                  <TableRow>
                                    ...
                                    <TableCell>
                                      <TextField
                                        ...
                                        onBlur={handleBlur}
                                        onChange={e =>
                                          changeText(e, ID, 'credit')
                                        }
                                        value={object.credit}
                                        fullWidth
                                        onKeyDown={e =>
                                          onKeyDown(e, ID, object)
                                        }
                                        inputProps={{
                                          readOnly: object.disableCreditField
                                        }}
                                      />
                                    </TableCell>
                                  </TableRow>
                      ...
                      <Box display="flex" justifyContent="flex-end" p={2}>
                        ...
                        {permissionList.isApproveButtonEnabled == true &&
                          !isUpdate ? (
                          <Button
                            color="primary"
                            id="btnRecord"
