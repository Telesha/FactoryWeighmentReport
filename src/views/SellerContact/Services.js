...
async function saveSellerContract(values) {
  let saveModel = {
    ...
    typeOfPack: values.typeOfPack,
    noOfPackages: parseFloat(values.noOfPackages),
    sampleAllowance: parseFloat(values.sampleAmount),
    packNetWeight: parseFloat(values.packNetWeight),
    netWeight: parseFloat(values.netAmount),
  ...
  const response = await CommonPost('/api/SellerContract/SaveSellerContract', null, saveModel);
  return response;
}
...
async function getSellerContractDetails(groupID, factoryID) {
  const response = await CommonGet('/api/SellerContract/GetSellerContractDetails', 'groupID=' + groupID + '&factoryID=' + factoryID);
  return response;
}
