...
export function CustomerCropBook({ cropBookArray, setCropBookArray, cusGeneralArray,
  ...
  async function saveCrop(values) {
    ...
    if (FormIsReadyToUpdate) {
      ...
      let selectedCropDetail = dataUpdate.find(x => x.registrationNumber === values.registrationNumber)
      let index = dataUpdate.map(x => x.registrationNumber).indexOf(values.registrationNumber)
      ...
      dataUpdate[index] = {
        ...
        accountTypeID: selectedCropDetail.accountTypeID,
        isDefault: selectedCropDetail.isDefault,
        registrationNumber: selectedCropDetail.registrationNumber,
      ...
      if (isDuplicate) {
        alert.error('Duplcate registration number');
      } else {
  ...
  function EditCustomerDisbursmentTypeDetails(index) {
      ...
      <Page className={classes.root} title="Crop Books Add Edit">
        <Container maxWidth={false}>
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                routeID: Yup.number().required('Route is required').min("1", 'Select a route'),
                accountTypeID: Yup.number().required('Account type is required').min("1", 'Select a account type'),
                registrationNumber: Yup.string().max(255).required('Registration number is required').matches(/^[1-9]{0,15}$/, 'Please enter valid registration number'),
                        ...
                        <Grid container spacing={3}>
                          <Grid item md={4} xs={12}>
                            ...
                            <TextField select
                              ...
                              variant="outlined"
                              // InputProps={{
                              //   readOnly: routeChange ? true : false,
                              // }}
