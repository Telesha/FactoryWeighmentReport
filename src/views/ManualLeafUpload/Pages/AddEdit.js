...
export default function LeafUploadAddEdit(props) {
  ...
  async function getLatestManualLeafUpload(groupID, factoryID, routeID, formattedDate) {
    ...
    if (response.length > 0) {
      setCropUploadDetailsList(response)
    }
    else {
          ...
          <Formik
            ...
            validationSchema={
              Yup.object().shape({
                ...
                regNo: Yup.string().required('Registration number is required').typeError('Please enter valid registration number').matches(/^[0-9]{0,15}$/, 'Please enter valid registration number'),
                leafType: Yup.number().min(1, "Please Select a Leaf Type").required('Collection Type is required'),
                amount: Yup.string().required('Leaf weight is required').matches(/^\d{1,5}(\.\d{1,2})?$/, 'Please enter valid leaf weight').test('amount', "Please provide valid leaf amount", val => val > 0),
              })
            }
