...
const screenCode = 'EMPLOYEECARDACTIVE';
export default function EmployeeCardActiveListing(props){
    ...
    useEffect(() => {
        if(empCardPrintList.groupID > 0){
            trackPromise(getEstateDetailsByGroupID());
        }
    }, [empCardPrintList.groupID]);

    useEffect(() => {
        if(empCardPrintList.estateID){
    ...
    async function getPermissions() {
        ...
        var isAuthorized = permissions.find(
          p => p.permissionCode == 'EMPLOYEECARDACTIVE'
        );
    ...
    async function SearchData(empCardPrintList){
        const cardDetails = await services.getEmpDetailsforCardActive(empCardPrintList.groupID,empCardPrintList.estateID,empCardPrintList.divisionID,empCardPrintList.employeeTypeID);
            if (cardDetails != null) {
    ...
    async function createDataForExcel(array) {
      ...
      async function createFile() {
        if(selectedEmpCardDetailsList.length > 0){
            ...
            var settings = {
            sheetName: 'Employee Card Active Details',
            fileName: 'Employee Card Active Details',
            writeOptions: {}
                }
            ...
            let dataA = [
            {
                sheet: 'Employee Card Active Details',
    ...
    async function handleCancel() {
      ...
      async function handleAtivate() {
        const response = await services.updateActiveCard(modalData);
    ...
    return(
        <Page className={classes.root} title="Employee Card Active">
            <Container maxWidth={false}>
                <Box mt={0}>
                    <Card>
                        <CardHeader title={'Employee Card Active'}/>
                        <PerfectScrollbar>
                            <Divider/>
                        ...
                        <Dialog open={open} onClose={handleClose}>
                                        <DialogTitle style={{ fontSize: '56px' }}>
                                          <Typography
                                                  color="textSecondary"
                                                  gutterBottom
                                                  variant="h4"
                                                >
                                                  <Box textAlign="left" >
                                                  Employee Card Active Confirmation
                                              </Box>
                                          </Typography>  
                                        </DialogTitle>
                                        <DialogContent>
                                        ...
                                        <DialogActions>
                                                    ...
                                                    style={{ marginRight: '1rem' }}
                                                    className={classes.colorRecord}
                                                    onClick={() => trackPromise(handleAtivate())}
                                                    size='small'
                                                    >
                                                    Active
                                                    </Button>
