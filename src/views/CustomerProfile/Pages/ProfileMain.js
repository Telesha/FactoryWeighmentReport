...
export default function CustomerProfileMain(props) {
    ...
    function OtherAccounts(item) {
        confirmAlert({
            title: 'Confirm To View Account ',
            message: "Do you need to view " + item.registrationNumber + " account",
            buttons: [
                {
                    label: 'Yes',
                    onClick: () => LoadOtherAccountToGrid(item.registrationNumber)
                },
                {
                    label: 'No',
                    onClick: () => handleClose()
    ...
    async function LoadOtherAccountToGrid(regNo) {
        setSearchReg({
            ...searchReg,
            RegNo: regNo
        })
